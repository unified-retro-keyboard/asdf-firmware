...
// Key 'a' in the test keymap selected by asdf_init(&kb)
#define KEY_A_ROW 1
#define KEY_A_COL 6
...
static void send_pending_codes(void)
{
  ...
  while (ASDF_INVALID_CODE != (code = test_next_code(&kb))) {
    asdf_send_code(&kb, (asdf_keycode_t) code);
  }
}
...
{
  asdf_install_platform(&kb, &fake_a.platform);
  asdf_install_platform(&kb, NULL);
...
void switching_platform_switches_hardware(void)
{
  ...
  asdf_install_platform(&kb, &fake_b.platform);
  scan(ASDF_DEBOUNCE_TIME_MS);
  send_pending_codes();
  TEST_ASSERT_EQUAL_INT(0, fake_a.num_sent);
...
// asdf_process(&kb) catches up elapsed ticks in one call: a key held for the
...
void process_catches_up_elapsed_ticks(void)
{
  ...
  asdf_process(&kb, ASDF_DEBOUNCE_TIME_MS);
  asdf_process(&kb, 1); // send the code queued by the last scan
  TEST_ASSERT_EQUAL_INT(1, fake_a.num_sent);
  TEST_ASSERT_EQUAL_INT('a', fake_a.sent[0]);
}
