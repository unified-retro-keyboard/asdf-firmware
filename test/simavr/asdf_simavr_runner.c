...
static int pred_capture_nonempty(void *ctx)
{
  (void) ctx;
  return cap_count() > 0;
}
static int pred_capture_even(void *ctx)
{
  (void) ctx;
  return !(cap_count() & 1u);
}
...
static args_t parse_args(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--target") && i + 1 < argc)
      ...
      a.gdb_port = atoi(argv[++i]);
    else if (!strcmp(argv[i], "--mode") && i + 1 < argc)
      a.mode = argv[++i];
    else if (!strcmp(argv[i], "--verbose"))
      a.verbose = 1;
    else if (!strcmp(argv[i], "--boot-only"))
      a.boot_only = 1;
    else {
  ...
  if (!a.target || !a.keymap || !a.elf_path)
    usage(argv[0]);
  if (!a.mode)
    a.mode = "events";
  if (strcmp(a.mode, "events") && strcmp(a.mode, "identity") && strcmp(a.mode, "string")
      && strcmp(a.mode, "out2") && strcmp(a.mode, "repeat") && strcmp(a.mode, "latency")) {
...
static const sim_keymap_test_t *pick_keymap(const char *name)
{
  if (!strcmp(name, "classic"))
    return &classic_test;
  if (!strcmp(name, "classic_caps"))
    return &classic_caps_test;
  if (!strcmp(name, "apple2"))
    return &apple2_test;
  if (!strcmp(name, "apple2_caps"))
    return &apple2_caps_test;
  if (!strcmp(name, "sol"))
    return &sol_test;
  if (!strcmp(name, "ace1000"))
    return &ace1000_test;
  return 0;
}
...
{
  if (!strcmp(name, "classic"))
    return &classic_identity_test;
  if (!strcmp(name, "classic_caps"))
    return &classic_caps_identity_test;
  if (!strcmp(name, "apple2"))
    return &apple2_identity_test;
  if (!strcmp(name, "apple2_caps"))
    return &apple2_caps_identity_test;
  if (!strcmp(name, "sol"))
    return &sol_identity_test;
  if (!strcmp(name, "ace1000"))
    return &ace1000_identity_test;
  return 0;
}
...
{
  if (!strcmp(name, "classic"))
    return &classic_string_test;
  if (!strcmp(name, "classic_caps"))
    return &classic_caps_string_test;
  if (!strcmp(name, "apple2"))
    return &apple2_string_test;
  if (!strcmp(name, "apple2_caps"))
    return &apple2_caps_string_test;
  if (!strcmp(name, "sol"))
    return &sol_string_test;
  if (!strcmp(name, "ace1000"))
    return &ace1000_string_test;
  return 0;
}
...
int main(int argc, char **argv)
{
  ...
  if (!io) {
    fprintf(stderr, "FAIL: no I/O map for %s\n", a.target);
    return 1;
  }
  ...
  if (!strcmp(a.mode, "events")) {
    ...
    if (!km) {
      fprintf(stderr, "FAIL: no test data for keymap %s\n", a.keymap);
      return 1;
    }
    ...
    for (int i = 0; i < km->num_events; i++) {
      ...
      if (e->hold_cycles > key_wait)
        key_wait = e->hold_cycles;
  ...
  if (!strcmp(a.mode, "identity")) {
    ...
    if ((int) unique != id->expected_len) {
      fprintf(stderr,
              "FAIL: %s/%s identity length mismatch: captured %zu (%zu unique), expected %d\n",
    ...
    for (int i = 0; i < id->expected_len; i++) {
      ...
      if (r.byte != (uint8_t) id->expected[i]) {
        fprintf(stderr, "FAIL: %s/%s identity byte[%d] = 0x%02x, expected 0x%02x\n", a.target,
                a.keymap, i, r.byte, (uint8_t) id->expected[i]);
        ...
        for (int j = i + 1; j < id->expected_len; j++) {
          ...
          if (!cap_pop(&rr))
            break;
          if (!cap_pop(&dd))
            break; /* ring parity violation - abort dump */
          fprintf(stderr, "  [%d] cycle=%" PRIu64 " byte=0x%02x %c (expected 0x%02x)\n", j,
                  rr.cycle, rr.byte, (rr.byte >= 0x20 && rr.byte < 0x7f) ? (char) rr.byte : '.',
    ...
    printf("OK: %s/%s identity %d bytes at cycle %" PRIu64 "\n", a.target, a.keymap,
           id->expected_len, (uint64_t) cpu->cycle);
    return 0;
  }
  ...
  if (!strcmp(a.mode, "string")) {
    ...
    for (int i = 0; i < st->num_steps; i++) {
      ...
      switch (step->type) {
        case SIM_STEP_KEY: {
          snprintf(what, sizeof what, "%s/step[%d]@(%d,%d)", a.keymap, i, step->row, step->col);
          matrix_press(step->row, step->col);
          if (sim_expect_byte_within(cpu, step->expected,
                                     (io->cpu_frequency_hz / 1000) * SIM_KEY_WAIT_MS, what)
              != 0) {
            vcd_end();
            return 1;
          }
    ...
    printf("OK: %s/%s passed %d string steps at cycle %" PRIu64 "\n", a.target, a.keymap,
           st->num_steps, (uint64_t) cpu->cycle);
    return 0;
  }
  ...
  if (!strcmp(a.mode, "out2")) {
    ...
    if (out2_edges != 2) {
      fprintf(stderr,
              ...
              a.target, a.keymap, io->out2_port, io->out2_bit, out2_edges, o2->trigger_key.row,
              o2->trigger_key.col);
      failed = 1;
    }
    if (out2_edges == 2) {
      ...
      if (width_ms < SIM_LONG_PULSE_MIN_MS || width_ms > SIM_LONG_PULSE_MAX_MS) {
        fprintf(stderr, "FAIL: %s/%s OUT2 pulse width %.2f ms, expected %d-%d ms\n", a.target,
                a.keymap, width_ms, SIM_LONG_PULSE_MIN_MS, SIM_LONG_PULSE_MAX_MS);
        failed = 1;
      }
    }
    if (led2_edges != 0) {
    ...
    vcd_end();
    printf(
      "OK: %s/%s OUT2 (P%c%d) toggled %u time(s), LED2 (P%c%d) untouched, "
      "at cycle %" PRIu64 "\n",
      a.target, a.keymap, io->out2_port, io->out2_bit, out2_edges, io->led2_port, io->led2_bit,
      (uint64_t) cpu->cycle);
    return 0;
  }
  ...
  if (!strcmp(a.mode, "latency")) {
    ...
    if (km) {
      for (int i = 0; i < km->num_events && !e; i++)
        if (km->events[i].with_modifier == SIM_MOD_NONE)
          e = &km->events[i];
    }
    if (!e) {
    ...
    for (int i = 0; i < SIM_LATENCY_SAMPLES; i++) {
      ...
      matrix_press(e->row, e->col);
      if (sim_run_until(cpu, pred_capture_nonempty, 0, cycles_per_ms * SIM_LATENCY_LIMIT_MS) != 1) {
        fprintf(stderr, "FAIL: %s/%s latency sample %d: no output within %d ms\n", a.target,
                a.keymap, i, SIM_LATENCY_LIMIT_MS);
        vcd_end();
        return 1;
      }
      asdf_cap_record_t r;
      ...
      if (sim_run_until(cpu, pred_capture_nonempty, 0, cycles_per_ms) == 1) {
        ...
        if (width_us < SIM_STROBE_MIN_US || width_us > SIM_STROBE_MAX_US) {
          fprintf(stderr, "FAIL: %s/%s strobe width %.1f us, expected %d-%d us\n", a.target,
                  a.keymap, width_us, SIM_STROBE_MIN_US, SIM_STROBE_MAX_US);
          vcd_end();
          return 1;
        }
        if (width_us < min_strobe_us)
          min_strobe_us = width_us;
        if (width_us > max_strobe_us)
          max_strobe_us = width_us;
      }

      uint64_t latency = r.cycle - pressed_at;
      if (latency < min_cycles)
        min_cycles = latency;
    ...
    if (max_cycles > cycles_per_ms * SIM_LATENCY_MAX_MS) {
      fprintf(stderr, "FAIL: %s/%s key latency max %.2f ms exceeds %d ms\n", a.target, a.keymap,
              (double) max_cycles / cycles_per_ms, SIM_LATENCY_MAX_MS);
      return 1;
    }
    printf(
      "OK: %s/%s key latency min %.2f ms, max %.2f ms over %d presses; "
      "strobe %.1f-%.1f us\n",
      a.target, a.keymap, (double) min_cycles / cycles_per_ms, (double) max_cycles / cycles_per_ms,
  ...
  if (!strcmp(a.mode, "repeat")) {
    ...
    if (rp->num_cols <= 0 || rp->num_cols > SIM_REPEAT_MAX_COLS || rp->minimum_count < 2) {
      fprintf(stderr, "FAIL: invalid repeat data for %s: num_cols=%d, minimum_count=%u\n", a.keymap,
              rp->num_cols, rp->minimum_count);
      vcd_end();
      return 1;
    }
    ...
    vcd_end();
    printf(
      "OK: %s/%s autorepeat uniform across %d columns "
      "(%u-%u emissions, tolerance %u) at cycle %" PRIu64 "\n",
      a.target, a.keymap, rp->num_cols, lo, hi, rp->tolerance, (uint64_t) cpu->cycle);
    return 0;
  }
