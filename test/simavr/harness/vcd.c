...
int vcd_begin(avr_t *cpu, const asdf_io_map_t *io, const char *path)
{
  ...
  for (int i = 0; i < io->led_count; i++) {
    avr_irq_t *led = avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->leds[i].port), io->leds[i].bit);
    char name[8];
  ...
  if (io->out2_port) {
    avr_irq_t *out2 = avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->out2_port), io->out2_bit);
    avr_vcd_add_signal(&g_vcd, out2, 1, "out2");
  }

  /* Row port (lo for family B, the only row port for family A) */
  avr_irq_t *row_irq =
    avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->row_port), IOPORT_IRQ_REG_PORT);
