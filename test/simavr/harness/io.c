...
{
  if (!cpu || slot < 0 || slot >= IO_WATCH_SLOTS || port == '\0' || bit < 0 || bit > 7)
    return -1;
...
{
  for (int i = 0; i < IO_WATCH_SLOTS; i++) g_watch_count[i] = 0;
}

unsigned io_watch_count(int slot)
{
  if (slot < 0 || slot >= IO_WATCH_SLOTS)
    return 0;
...
{
  if (slot < 0 || slot >= IO_WATCH_SLOTS || edge < 0 || edge > 1)
    return 0;
...
{
  avr_irq_t *pin_irq =
    avr_io_getirq(g_cpu, AVR_IOCTL_IOPORT_GETIRQ(g_io->col_port), g_io->col_bit_serial);
...
static void on_col_clock_familyA(struct avr_irq_t *irq, uint32_t value, void *param)
{
  ...
  if (now && !g_familyA_last_clock) { /* rising edge */
    if (g_familyA_in_load_phase) {
      ...
      if (g_active_row_familyA >= 0 && g_active_row_familyA < MATRIX_MAX_ROWS) {
...
void io_wire_output(avr_t *cpu, const asdf_io_map_t *io)
{
  ...
  avr_irq_t *port_irq =
    avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->data_port), IOPORT_IRQ_REG_PORT);
  ...
  avr_irq_t *strobe_irq =
    avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->strobe_port), io->strobe_bit);
  ...
  if (!io->col_parallel) {
    avr_irq_t *col_irq =
      avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->col_port), io->col_bit_serial);
...
{
  if (row < 0 || row >= MATRIX_MAX_ROWS)
    return;
  if (col < 0 || col >= MATRIX_MAX_COLS)
    return;
...
void io_wire_input(avr_t *cpu, const asdf_io_map_t *io)
{
  ...
  if (io->col_parallel) {
    ...
    avr_irq_t *lo = avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->row_port), IOPORT_IRQ_REG_PORT);
    avr_irq_register_notify(lo, on_row_port_familyB, (void *) (intptr_t) 0);

    if (io->row_port_hi) {
      avr_irq_t *hi =
        avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->row_port_hi), IOPORT_IRQ_REG_PORT);
    ...
    avr_irq_t *row = avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->row_port), IOPORT_IRQ_REG_PORT);
    avr_irq_register_notify(row, on_row_port_familyA, 0);

    /* COLMODE on PORTB[col_mode_bit]: bit-level notifier. */
    avr_irq_t *mode =
      avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->col_mode_port), io->col_mode_bit);
    avr_irq_register_notify(mode, on_col_mode_familyA, 0);

    /* COLCLK on PORTB[col_load_clock_bit]: bit-level notifier. */
    avr_irq_t *clk =
      avr_io_getirq(cpu, AVR_IOCTL_IOPORT_GETIRQ(io->col_load_clock_port), io->col_load_clock_bit);
