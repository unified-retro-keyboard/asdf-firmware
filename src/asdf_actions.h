...
// One key: the action on press and the action on release. An all-zero key does
// nothing, so keys left out of a matrix initializer do nothing.
typedef struct {
...
// The built-in actions, numbered from 0. Numbers from ASDF_KEYMAP_ACTIONS on
// are free for keymap-provided actions (see the action table, below).
typedef enum {
  ACTION_NOTHING = 0,
...
// The action table, indexed by action number, in flash. The build supplies it
// with its keymaps (the keymap action table in Keymaps/, or the test keymaps),
// filling every entry: unused entries are asdf_action_nothing, the built-in
// entries are ASDF_BUILTIN_ACTIONS, and the rest are keymap-provided actions.
extern const asdf_action_fn_t FLASH asdf_action_table[ASDF_NUM_ACTION_SLOTS];

// Built-in action functions (asdf_actions.c).
...
// Key initializers for keymap matrices. Every KEY_ macro takes one parameter:
// the code, bit, output, or other value its action uses. Keys whose actions
// need no value ignore it, and are written with 0, for example KEY_SHIFT(0).
// In the YAML key matrices the parameter may be omitted, and is then 0.
...
// OUTPUTS: none
// DESCRIPTION: performs an action on the keyboard.
...
// autorepeat selection. On a keymap switch, the press actions of held keys are
// re-applied only if they are configuration actions.
